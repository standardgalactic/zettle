...
# -----------------------------------------------------------------------------
# process_files: Splits each .md file into chunks and summarizes them
# -----------------------------------------------------------------------------
process_files() {
    local dir="$1"
    echo "Processing directory: $dir"

    # Iterate over each .md file in this directory
    for file in "$dir"/*.md; do
        # Skip if no .md file exists (glob didn’t match)
        [ -e "$file" ] || continue

        # Double-check it's a regular file
        if [ -f "$file" ]; then
            # Example of skipping overview.md if needed
            local file_name
            file_name="$(basename "$file")"
            if [ "$file_name" = "overview.md" ]; then
...
# Main Execution
# -----------------------------------------------------------------------------
process_files "$main_dir"            # Process .md files in the main directory
